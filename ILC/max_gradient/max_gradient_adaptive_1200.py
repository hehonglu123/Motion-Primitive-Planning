...
def main():
	...
	ms = MotionSend(robot1=robot)
	breakpoints,primitives,p_bp,q_bp=ms.extract_data_from_cmd(cmd_dir+'command2.csv')

	###extension
	p_bp,q_bp=ms.extend(robot,q_bp,primitives,breakpoints,p_bp,extension_start=100,extension_end=150)
	...
	for i in range(iteration):
		...
		breakpoint_interp_2tweak_indices_multipeak=[]
		de_dp_multipeak=[]
		...
		for peak in peaks:
			...
			de_dp_multipeak.append(de_dp)
			breakpoint_interp_2tweak_indices_multipeak.append(breakpoint_interp_2tweak_indices)
		...
		for x in range(4):
			...
			for pp in range(len(peaks)):
				peak=peaks[pp]
				p_bp_temp, q_bp_temp=ilc.update_bp_xyz(p_bp_temp,q_bp_temp,de_dp_multipeak[pp],error[peak],breakpoint_interp_2tweak_indices_multipeak[pp],alpha)
