...
def main():
	...
	for i in range(iteration):
		...
		for bp_idx in range(len(p_bp)):
			...
			_,curve_original_idx=calc_error(curve_exe[bp_exe_idx],curve[:,:3])	###find closest point on curve_original to curve_exe[bp]
			error_ori_bp=get_angle(curve[curve_original_idx,3:],curve_exe_R[bp_exe_idx,:,-1])

			curve_original_indices.append(curve_original_idx)			
			error_ori_bps.append(error_ori_bp)
			###gradient direction
			kd_temp=np.cross(curve_exe_R[bp_exe_idx,:,-1],curve[curve_original_idx,3:])
			kd_temp=kd_temp/np.linalg.norm(kd_temp)
			w_d.append(kd_temp*error_ori_bp/2.)
			###p_bp temp for numerical gradient
			R_bp=robot.fwd(q_bp[bp_idx][0]).R
			R_bp_temp=rot(kd_temp,error_ori_bp/2.)@R_bp
			q_bp_temp[bp_idx][0]=car2js(robot,q_bp[bp_idx][0],p_bp_temp[bp_idx][0],R_bp_temp)[0]
		...
		for bp_idx in range(len(p_bp)):
			...
			de_dps.append(new_error-error_ori_bps[bp_idx])

		###find correct gamma
		de_dps=np.reshape(de_dps,(-1,1))
		gamma=np.linalg.pinv(de_dps)@error_ori_bps
