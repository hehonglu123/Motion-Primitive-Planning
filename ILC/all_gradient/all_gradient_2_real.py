...
def main():
	...
	for i in range(iteration):
		...
		bp_exe_indices=np.zeros((len(p_bp),2))		###breakpoints on curve_exe
		...
		for bp_idx in range(len(p_bp)):
			for bp_sub_idx in range(len(p_bp[bp_idx])):
				...
				if bp_idx==0 and bp_sub_idx==0:
					...
					# print('start',blended_idx,error_bp,bp_exe_idx)
		...
		for bp_idx in range(len(p_bp)):
			for bp_sub_idx in range(len(p_bp[bp_idx])):
				if (bp_idx==0 and bp_sub_idx==0) or (bp_idx==len(p_bp)-1 and bp_sub_idx==len(p_bp[bp_idx])-1):
					gamma=1
				else:
				...
				p_bp[bp_idx][bp_sub_idx]+=gamma*error_bps_v[bp_idx][bp_sub_idx]
				q_bp[bp_idx][bp_sub_idx]=car2js(robot,q_bp[bp_idx][bp_sub_idx],p_bp[bp_idx][bp_sub_idx],robot.fwd(q_bp[bp_idx][bp_sub_idx]).R)[0]


		# p_bp_np=np.array([item[0] for item in p_bp])      ###np version, avoid first and last extended points
		# ax.scatter(p_bp_np[:,0], p_bp_np[:,1], p_bp_np[:,2], color='blue',label='adjusted breakpoints')
		# bp_original_np=np.array([curve[int(item[0]),:3] for item in curve_original_indices])  
		# ax.scatter(bp_original_np[:,0], bp_original_np[:,1], bp_original_np[:,2], c='red',label='bp_star')
		# bp_exe_np=np.array([curve_exe[int(item[0])] for item in bp_exe_indices])  
		# ax.scatter(bp_exe_np[:,0], bp_exe_np[:,1], bp_exe_np[:,2], c='gray',label='bp_exe')

		# ax.set_xlim3d(0, 1000)
		# ax.set_ylim3d(-1400, -400)
