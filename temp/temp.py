...
R=np.array([[-1,0,0],
			[0,1,0],
			[0,0,-1]])
p1=np.array([400,0,300])
p2=np.array([500,0,300])
...
def linear(q1,q2):
	...
	pose_start=robot.fwd(q1)
	cf=quadrant(q1,robot)
	t=robtarget(pose_start.p, R2q(pose_start.R), confdata(cf[0],cf[1],cf[2],cf[3]),[0]*6)
	mp.MoveL(t,v10,fine)
...
def rotate(q1,theta):
	v_temp=speeddata(100,9,9999999,999999)
	mp = MotionProgram()
