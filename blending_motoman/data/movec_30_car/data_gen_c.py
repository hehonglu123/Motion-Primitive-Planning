...
###########save to csv####################
df=DataFrame({'breakpoints':np.array([0,int((len(curve)+1)/2),len(curve)]),'primitives':['movej_fit','movec_fit','movec_fit'],'p_bp':[[curve[0]],[curve[int(len(curve)/4)],curve[int((len(curve)+1)/2)]],[curve[int(3*len(curve)/4)],curve[-1]]],'q_bp':[[curve_js[0]],[curve_js[int(len(curve)/4)],curve_js[int((len(curve)+1)/2)]],[curve_js[int(3*len(curve)/4)],curve_js[-1]]]})
