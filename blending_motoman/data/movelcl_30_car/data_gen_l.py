...
curve[start_idx:end_idx+1],R_all[start_idx:end_idx+1]=movec_insertion(curve[start_idx:end_idx+1],curve[start_idx-1],curve[end_idx+1],R_all[start_idx-1],R_all[end_idx+1],curve[start_idx]-curve[start_idx-1],curve[end_idx+1]-curve[end_idx])
###update js
...
plt.show()
# ###########save to csv####################
df=DataFrame({'breakpoints':np.array([0,start_idx,end_idx+1,len(curve)]),'primitives':['movej_fit','movel_fit','movec_fit','movel_fit'],'points':[[q_init],[curve[start_idx]],[curve[bp_idx],curve[end_idx]],[curve[-1]]]})
