...
from gpu_utils.circular_fit_gpu import *
import pandas as pd
...
class Primitive(object):
    ...
    def move_l_fit(self, curve, p=[]):
        if len(p) == 0:
            A = torch.vstack((torch.ones(len(curve), device=device), torch.arange(0, len(curve), device=device))).T
            b = curve
            res = torch.lstsq(A, b).solution
            start_point = res[0]
        ...
        curve_fit = torch.dot(np.arange(0, len(curve)).reshape(-1, 1), slope) + start_point

        max_error = torch.max(torch.norm(curve - curve_fit, dim=1))
