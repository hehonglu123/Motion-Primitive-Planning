...
class greedy_fit(fitting_toolbox):
	...
	def fit_under_error(self,max_error_threshold,max_ori_threshold=np.radians(3)):
		...
		while self.breakpoints[-1]<len(self.curve)-1:
			...
			self.max_error = max(self.max_error, max_errors[primitives_choices[-1]])
			self.max_ori_error = max(self.max_ori_error, max_ori_errors[primitives_choices[-1]])
...
def main():
	...
	for i in range(201):
		...
		data = read_csv(lam_file, header=None)
		lam = data.values[-1][0]
		...
		curve_idx.append(i)
		n_primitives.append(len(primitives_choices))
		...
		ax.plot3D(greedy_fit_obj.curve[:, 0], greedy_fit_obj.curve[:, 1], greedy_fit_obj.curve[:, 2], 'red', linewidth=10)
		plt.title("#BP: {}; #Primitive: {}; Length: {}; Error {:.3f}; OriError {:.3f}".format(len(breakpoints), len(primitives_choices), len(greedy_fit_obj.curve_fit), greedy_fit_obj.max_error, greedy_fit_obj.max_ori_error))
		plt.savefig("plots/poly_greedy/{}.jpg".format(i), dpi=500)
		# plt.show()
		plt.close()
		...
		print(i, len(breakpoints), len(primitives_choices), len(points))
