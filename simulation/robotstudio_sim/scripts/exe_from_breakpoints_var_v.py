...
def main():
    ...
    logged_data=ms.exec_motions(robot1,breakpoints1,primitives1,p_bp1,q_bp1,v1_all,z)
    # Write log csv to file
    with open("recorded_data/curve_exe_v"+str(vd_relative)+'_z'+str(zone)+'.csv',"w") as f:
        f.write(logged_data)
    StringData=StringIO(logged_data)
    df = read_csv(StringData, sep =",")
    lam, curve_exe,curve_exe_R,curve_exe_js, speed, timestamp = ms.logged_data_analysis(robot1,df,realrobot=True)
