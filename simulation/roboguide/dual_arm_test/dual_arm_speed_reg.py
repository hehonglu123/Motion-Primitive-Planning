...
def main():
    ...
    s=target_speed # mm/sec in leader frame
    z=100 # CNT100
    ...
    # coord at each motion 
    coord_primitives=np.ones(len(primitives1))
    # coord_primitives=np.zeros(len(primitives1))

    # coord at each motion segments
    speed_seg = np.ones(len(primitives1))*s
    draw_error_max=None
    draw_speed_max=None
    iteration=30
    for i in range(iteration):
        ...
        logged_data=ms.exec_motions_multimove(robot1,robot2,primitives1,primitives2,p_bp1,p_bp2,q_bp1,q_bp2,speed_seg,z,coord_primitives)
        ...
        if save_fig:
            plt.savefig('data/speed_reg_'+test_type+'/tarvel_'+str(target_speed)+'_error_speed_'+str(i))
            plt.clf()
        else:
            plt.show()
        ...
        if plot_traj:
            ...
            plt.legend()
            if save_fig:
                plt.savefig('data/speed_reg_'+test_type+'/tarvel_'+str(target_speed)+'_traj_'+str(i))
                plt.clf()
            else:
                plt.show()
        ...
        for m in range(1,len(lam_bp)):
            ...
            if np.argmin(np.abs(lam-lam_bp[m]))<=np.argmin(np.abs(lam-lam_bp[m-1])):
                continue
            segment_avg=np.average(speed[np.argmin(np.abs(lam-lam_bp[m-1])):np.argmin(np.abs(lam-lam_bp[m]))])
            ###cap above 100m/s for robot2
            speed_seg[m+step_start]+=speed_alpha*(target_speed-segment_avg)
            # speed_seg[m]=max(s2[m],100)
        speed_seg[:step_start]=speed_seg[step_start+1]
        speed_seg[step_end:]=speed_seg[step_end-1]
        print("New Speed")
        print(speed_seg)
