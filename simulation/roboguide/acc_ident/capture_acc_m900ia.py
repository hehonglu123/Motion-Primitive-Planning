...
def exec(q_d,joint):
	...
	if joint==1:
		if q_d[2] > radians(15):
			lower_limit[1] = robot.lower_limit[1]+(q_end[2]-radians(15))
	elif joint==2:
		if q_d[1] < radians(-25):
			upper_limit[2] = robot.upper_limit[2]+(q_d[1]-radians(-25))

	if np.abs(q_d[joint]-lower_limit[joint]) < np.abs(q_d[joint]-upper_limit[joint]):
		q_init[joint]-=0.1
		q_end[joint]+=1

		if q_init[1] < radians(-25):
			upper_limit[2] = robot.upper_limit[2]+(q_init[1]-radians(-25))
		...
		# while np.any(q_end<lower_limit) or np.any(q_end>upper_limit):
			...
			if q_end[2] > radians(15):
				...
				lower_limit[1] = robot.lower_limit[1]
			# print(q_end[joint])
			if dang < 0.3:
		...
		if q_end[1] < radians(-25):
			upper_limit[2] = robot.upper_limit[2]+(q_end[1]-radians(-25))

		if q_init[2] > radians(15):
			lower_limit[1] = robot.lower_limit[1]+(q_init[2]-radians(15))
		...
		# while np.any(q_end<lower_limit) or np.any(q_end>upper_limit):
			...
			if q_end[1] < radians(-25):
				...
				upper_limit[2] = robot.upper_limit[2]
			# print(q_end[joint])
			if dang < 0.3:
				print("dang too small")
				print(lower_limit)
...
for q2 in np.arange(q2_test_lower,q2_test_upper,resolution):
	...
	for q3 in np.arange(q3_test_lower+resolution*0,q3_test_upper,resolution):
