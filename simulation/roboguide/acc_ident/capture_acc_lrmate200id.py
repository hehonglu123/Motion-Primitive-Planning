...
# robot=m710ic(d=50)
# robot=m10ia(d=50)
robot=lrmate200id(Ry(np.radians(90)),p_tool=np.array([0,0,0]))
resolution=0.05 ###rad

dict_table={}

q2_up_sample = np.radians(np.arange(60,145+0.1,5))
q3_low_sample = np.radians(np.array([-70,-68,-65.5,-62.5,-59,-54.5,-50,-44.5,-39,-32.5,-25.5,-18,-10,-2,6.5,15,24,33]))
fq3low_q2 = interp1d(q2_up_sample,q3_low_sample)
fq2up_q3 = interp1d(q3_low_sample,q2_up_sample)

q2_low_sample = np.radians(np.arange(-100,-75+0.1,5))
q3_up_sample = np.radians(np.array([180,185,190,195,200,205]))
...
q2_test_lower = robot.lower_limit[1]+resolution*65
...
for q2 in np.arange(q2_test_lower,q2_test_upper,resolution):
	...
	for q3 in np.arange(q3_test_lower,q3_test_upper,resolution):
		...
		with open('lrmate200id/acc1.txt','w+') as f:
			f.write(str(dict_table))
		pickle.dump(dict_table, open('lrmate200id/acc1.pickle','wb'))
