...
pickle.dump(dic, open(robot_name+'/'+robot_name+'_acc.pickle','wb'))

dic_comp={}
for key, value in dic.items():
   dic_comp[(key[0],-key[1])]=value

pickle.dump(dic_comp, open(robot_name+'/'+robot_name+'_acc_compensate.pickle','wb'))
...
for key, value in dic.items():
   ...
   q1_acc_n.append(value[1])
   q2_acc_p.append(value[2])
   q2_acc_n.append(value[3])
   q3_acc_p.append(value[4])
   q3_acc_n.append(value[5])
...
surf = ax.plot_trisurf(x, y, q1_acc_p, linewidth=0, antialiased=False)
...
fig = plt.figure()
ax = fig.add_subplot(111, projection='3d')
surf = ax.plot_trisurf(x, y, q1_acc_n, linewidth=0, antialiased=False)
ax.set_xlabel('q2 (rad)')
ax.set_ylabel('q3 (rad)')
ax.set_zlabel('q1 acc (rad/s^2)')
