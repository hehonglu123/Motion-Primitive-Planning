...
for speed in all_speed:
    ...
    tp = TPMotionProgram()
    robt1 = joint2robtarget(curve_js[int((len(curve)+1)/2)],robot,1,1,utool_num)
    robt2 = joint2robtarget(curve_js[-1],robot,1,1,utool_num)
    tp.moveC(robt1,robt2,speed,'mmsec',-1)
    
    # execute 
    res = client.execute_motion_program(tp)
    # Write log csv to file
    with open(save_dir+"movec_"+str(speed)+".csv","wb") as f:
        f.write(res)