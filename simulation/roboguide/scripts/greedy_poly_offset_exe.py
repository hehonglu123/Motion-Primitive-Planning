...
def main():
    ...
    data = read_csv("../../../data/fanuc/analytical_expression_offset.csv", names=col_names)
    ...
    data = read_csv("../../../data/fanuc/m900ia_curve_js_sol4_offset_poly.csv", names=col_names)
    ...
    # save directory
    save_dir='../data/greedy_poly_offset/'
    ...
    for zone in all_zones:
        ...
        for mo_id in range(0,len(breakpoints)-1):
            ...
            if this_motion == 'movej_fit':
                ...
                # robt2 = jointtarget(1,1,utool_num,np.degrees(curve_fit_js[bp-1]),[0]*6) # goal
                # robt1 = jointtarget(1,1,utool_num,np.degrees(curve_fit_js[int((bp-breakpoints[mo_id])/2+breakpoints[mo_id])]),[0]*6) # mid point
