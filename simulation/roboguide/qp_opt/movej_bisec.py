    ...
    while True:
        ...
        for i in range(1,len(robt_all)-1):
            if motion_all[i-1]=='L':
                ...
                # tp.moveJ(robt,speed,'msec',zone)
        # end with zone = -1 (fine)
        if motion_all[i-1]=='L':
            robt = robt_all[i]
            ...
            robt_pass = robt_all[i][0]
            robt = robt_all[i][1]
            tp.moveC(robt_pass,robt,speed,'mmsec',-1)
        else:
            robt = robt_all[i]
            tp.moveJ(robt,speed,'%',-1)
            # tp.moveJ(robt,speed,'msec',-1)
        # execute 
...
for total_seg in all_total_seg:
    print("Total Seg:",total_seg)
    step=int((len(curve_js)-1)/total_seg)
    all_robt = [jointtarget(1,1,utool_num,np.degrees(curve_js[0]),[0]*6)]
    all_move = []
    for i in range(step,len(curve_js)-1,step):
        robt = jointtarget(1,1,utool_num,np.degrees(curve_js[i]),[0]*6)
        all_robt.append(robt)
        all_move.append('J')
    robt = jointtarget(1,1,utool_num,np.degrees(curve_js[-1]),[0]*6)
    all_robt.append(robt)
    all_move.append('J')
    
    this_data_dir = data_dir+'seg_'+str(total_seg)

    des_speed_min = 10.
    duration_up = 1000./total_seg/des_speed_min*1000
    run_robot_bisec(all_robt,all_move,duration_up,1,this_data_dir)