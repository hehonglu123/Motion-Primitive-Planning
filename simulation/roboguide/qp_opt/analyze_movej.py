...
for total_seg in all_total_seg:
    ...
    # the executed result
    with open(this_data_dir+'/error.npy','rb') as f:
        error = np.load(f)
    with open(this_data_dir+'/normal_error.npy','rb') as f:
        angle_error = np.load(f)
    with open(this_data_dir+'/speed.npy','rb') as f:
        act_speed = np.load(f)
    with open(this_data_dir+'/lambda.npy','rb') as f:
        lam_exec = np.load(f)
    ...
    ax2.legend(loc=7)
    # plt.show()
    plt.savefig(img_dir+'speed_error_seg_'+str(total_seg)+'.png')
    plt.clf()
    ...
    # data=read_csv(this_data_dir+"/curve_js_exe.csv",names=col_names)
    # q1=data['J1'].tolist()[1:]
    # q2=data['J2'].tolist()[1:]
    # q3=data['J3'].tolist()[1:]
    # q4=data['J4'].tolist()[1:]
