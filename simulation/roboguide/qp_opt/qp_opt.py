...
the_basis = Monomial(domain_range=(0,1),n_basis=bases_num)
...
for i in range(50):
    ...
    H = (1/this_lam[-1])*ldot*np.matmul(dldp,np.matmul(Jp_mat,basis_dmat))
    P = np.dot(np.reshape(H,(len(H),1)),np.reshape(H,(1,len(H))))+0.01*np.eye(len(H))
    f = -ldot_tar*H
    ...
    ## qdot constraint
    G=np.vstack((G,basis_dmat*ldot))
    h = np.append(h,qdot_path)
    G=np.vstack((G,-basis_dmat*ldot))
    h = np.append(h,qdot_path)
    # ## qddot constraint
    G=np.vstack((G,basis_ddmat*ldot))
