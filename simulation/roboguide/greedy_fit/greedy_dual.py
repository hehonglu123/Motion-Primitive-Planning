...
class greedy_fit(fitting_toolbox):
    def __init__(self,robot1,robot2,curve_js1,curve_js2,base2_p,base2_R,min_length,max_error_threshold,max_ori_threshold=np.radians(3)):
        ...
        # self.primitives1={'movel_fit':self.movel_fit,'movej_fit':self.movej_fit,'movec_fit':self.movec_fit}
        self.primitives1={'movel_fit':self.movel_fit,'movec_fit':self.movec_fit}
        self.primitives2={'movel_fit':self.movel_fit}
...
def main():
    ...
    for obj_type in all_objtype:
        ...
        ### curve directory
        curve_dir=os.getcwd()+'/../../../data/'+obj_type+'/'
        ### data directory
        data_dir=os.getcwd()+'/../data/'+obj_type+'/dual_arm_de/'
        print(data_dir)

        ## robot
        toolbox_path = os.getcwd()+'/../../../toolbox/'
        ...
        robot2=robot_obj('FANUC_lrmate200id',toolbox_path+'robot_info/fanuc_lrmate200id_robot_default_config.yml',tool_file_path=data_dir+'tcp.csv',base_transformation_file=data_dir+'base.csv',acc_dict_path=toolbox_path+'robot_info/lrmate200id_acc_compensate.pickle',j_compensation=[1,1,-1,-1,-1,-1])
        H_200id=np.loadtxt(data_dir+'base.csv',delimiter=',')
        base2_R=H_200id[:3,:3]
        base2_p=H_200id[:-1,-1]
        ...
        for max_error_threshold in all_max_error_thres:
            ...
            Path(data_dir+'greedy'+str(max_error_threshold)).mkdir(exist_ok=True)
            output_dir=data_dir+'greedy'+str(max_error_threshold)+'/'
    
            relative_path,lam_relative_path,lam1,lam2,curve_js1,curve_js2=initialize_data(obj_type,curve_dir,data_dir,robot1,robot2)

            min_length=20
            greedy_fit_obj=greedy_fit(robot1,robot2,curve_js1[::1],curve_js2[::1],base2_p,base2_R,min_length,max_error_threshold)
            ...
            df.to_csv(output_dir+'curve_fit1.csv',header=True,index=False)
            DataFrame(greedy_fit_obj.curve_fit_js1).to_csv(output_dir+'curve_fit_js1.csv',header=False,index=False)
            ...
            df.to_csv(output_dir+'curve_fit2.csv',header=True,index=False)
            DataFrame(greedy_fit_obj.curve_fit_js2).to_csv(output_dir+'curve_fit_js2.csv',header=False,index=False)
