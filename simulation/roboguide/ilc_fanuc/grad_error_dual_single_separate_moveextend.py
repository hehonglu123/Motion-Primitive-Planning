    ...
    for i in range(start_iteration,iteration):
        ...
        while True:
                ...
                if i < speed_iteration+start_iteration:
                    ...
                    if max(error_prev) > max_error_all_thres:
                        ...
                        for j in range(0,len(p_bp1)):
                            p_bp1_update[j][-1] = np.array(p_bp1[j][-1]) + alpha*p_bp1_error_dir[j]
                            bp1_R = robot1.fwd(q_bp1[j][-1]).R

                            #### get the new R
                            bp1_R[:,2] = (bp1_R[:,2] + alpha*p_bp1_ang_error_dir[j])/np.linalg.norm((bp1_R[:,2] + alpha*p_bp1_ang_error_dir[j]))
            ...
            if not alpha_flag: # dont draw again is alpha flag raised
                ...
                dfcmd2=DataFrame({'primitives':primitives2,'points':p_bp2,'q_bp':q_bp2,'speed':s2_movel_update})
