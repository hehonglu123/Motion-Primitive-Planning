...
def main():
    ...
    save_fig = False
    all_use_iteration=[42,43,44]
    for use_iteration in all_use_iteration:
        print("Using iteration:",use_iteration)
        ########## read data ##############
        cmd_folder = data_dir+'results_1000_dual_arm_first_test/'
        _,primitives1,p_bp1,q_bp1,s1_movel=ms.extract_data_from_cmd(os.getcwd()+'/'+cmd_folder+'command_arm1_'+str(use_iteration)+'.csv')
        _,primitives2,p_bp2,q_bp2,s2_movel=ms.extract_data_from_cmd(os.getcwd()+'/'+cmd_folder+'command_arm2_'+str(use_iteration)+'.csv')
