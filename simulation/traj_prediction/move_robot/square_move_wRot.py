...
side_l = 200
rot_ang = 15
quat4 = rox.R2q(rox.rot([0,1,0],pi/2))
quat1 = rox.R2q(np.matmul(rox.rot([0,1,0],pi/2),rox.rot([1,0,0],radians(rot_ang))))
quat2 = rox.R2q(np.matmul(np.matmul(rox.rot([0,1,0],pi/2),rox.rot([1,0,0],radians(rot_ang))),rox.rot([0,1,0],radians(rot_ang))))
quat3 = rox.R2q(np.matmul(rox.rot([0,1,0],pi/2),rox.rot([0,1,0],radians(rot_ang))))
conf = [0,0,0,1]
v_profile=v50
zone=z10
ang = radians(90)/2
...
log = send_square(p1,p2,p3,p4,quat1,quat2,quat3,quat4,conf,v_profile,zone)

with open("square_move_v50_wRot_left.csv","wb") as f:
    f.write(log)
...
with open("square_move_v50_wRot_middle.csv","wb") as f:
    f.write(log)
...
with open("square_move_v50_wRot_right.csv","wb") as f:
    f.write(log)
...
with open("square_move_v500_wRot0_left.csv","wb") as f:
    f.write(log)
...
with open("square_move_v500_wRot0_middle.csv","wb") as f:
    f.write(log)
...
with open("square_move_v500_wRot0_right.csv","wb") as f:
    f.write(log)
