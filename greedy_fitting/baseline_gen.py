...
def main():
	...
	for threshold in thresholds:
		...
		df.to_csv(data_dir+'baseline/'+str(threshold)+'/command.csv',header=True,index=False)
		...
		df.to_csv(data_dir+'baseline/'+str(threshold)+'/curve_fit.csv',header=True,index=False)
		DataFrame(greedy_fit_obj.curve_fit_js).to_csv(data_dir+'baseline/'+str(threshold)+'/curve_fit_js.csv',header=False,index=False)
